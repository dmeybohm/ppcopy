...
#   ppcopy_port=278     — optional parallel port base address (hex) to pass on
#
...
for param in $(cat /proc/cmdline); do
    case "$param" in
        ...
        ppcopy_port=*) PORT="${param#ppcopy_port=}" ;;
    esac
done
...
case "$ROLE" in
    writer)
        echo "ppcopy-test: writer starting, file=$FILE port=${PORT:-default}" > /dev/ttyS0
        /ppwrite-i386 "$FILE" $PORT
        RC=$?
        if [ "$RC" -eq 0 ]; then
        ...
        ;;
    reader)
        echo "ppcopy-test: reader starting, expected=$FILE port=${PORT:-default}" > /dev/ttyS0
        /ppread-i386 $PORT > /received.bin
        RC=$?
        if [ "$RC" -ne 0 ]; then
